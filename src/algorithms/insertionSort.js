export default async function insertionSort(ref, array) {
  ...
  array.forEach((num, index) => {
    ref.bars[index] = { value: num.value, color: ref.primary };
  });
  ...
  for (let i = 1; i < array.length; i++) {
    ...
    ref.bars[j] = { value: a, color: ref.compare };
    ref.bars[j - 1] = { value: b, color: ref.compare };
    ...
    while (j > 0 && array[j].value < array[j - 1].value) {
      ...
      let { value: a } = array[j];
      let { value: b } = array[j - 1];
      ref.bars[j] = { value: b, color: ref.sorted };
      ref.bars[j - 1] = { value: a, color: ref.compare };

      // highlight the next value to compare to
      if (array[j - 2]) {
        let { value } = array[j - 2];
        ref.bars[j - 2] = { value, color: ref.compare };
      }
    ...
    if (array[j]) {
      let { value: a } = array[j];
      ref.bars[j] = { value: a, color: ref.sorted };
    }
    if (array[j - 1]) {
      let { value: b } = array[j - 1];
      ref.bars[j - 1] = { value: b, color: ref.sorted };
    }
  }
}
